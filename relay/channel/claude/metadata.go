...
func masqueradeMetadata(raw json.RawMessage, channelID int, channelHash string, maxSessions int, apiKey string) (json.RawMessage, string, string) {
	...
	return pool.MasqueradeMetadata(raw, apiKey)
}
...
func MasqueradeMetadataInBody(body []byte, channelID int, channelHash string, maxSessions int, apiKey string) ([]byte, string, string) {
	payload := make(map[string]json.RawMessage)
	if err := json.Unmarshal(body, &payload); err != nil {
		return body, "<empty>", ""
	}

	maskedMetadata, originalUserID, maskedUserID := masqueradeMetadata(payload["metadata"], channelID, channelHash, maxSessions, apiKey)
