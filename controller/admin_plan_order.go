...
func ManualCompletePlanOrder(c *gin.Context) {
	...
	go affHookForPlanOrder(order.Id, order.UserId, order.FinalPrice, order.PlanQuota, order.PlanType, common.GetTimestamp()*1000, "", "")
...
func ManualCompleteTopupOrder(c *gin.Context) {
	...
	if completed, _ := model.GetTopupOrderById(orderId); completed != nil {
		go affHookForTopupOrder(completed.Id, completed.UserId, completed.FinalPrice, completed.Amount, completed.PaidAt, "", "")
	}
