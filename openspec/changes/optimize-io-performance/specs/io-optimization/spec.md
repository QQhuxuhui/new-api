...
### Requirement: GetAllChannels performance in tag mode

系统 **SHALL** 修改Tag模式下的渠道列表查询，使用批量查询替代循环查询。

#### Scenario: List channels in tag mode with 100 tags

**Given** 系统中有100个不同的Tag
**When** 请求 `GET /api/channel?tag_mode=true`
**Then** 数据库查询次数 <= 3
**And** 响应时间 < 200ms (原先可能 > 2000ms)
**And** 返回数据与原实现一致

---

### Requirement: GetAllChannelsHealth performance

系统 **SHALL** 修改全部渠道健康状态查询，使用批量Redis操作。

#### Scenario: Get health for 100 channels

**Given** 系统中有100个渠道
**When** 请求 `GET /api/channel/health`
**Then** Redis操作通过Pipeline批量执行
**And** Redis网络往返次数 <= 5 (原先可能 > 1700)
**And** 响应时间 < 500ms
