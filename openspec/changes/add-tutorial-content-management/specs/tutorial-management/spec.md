# Spec: Tutorial Management

## Overview

Administrators MUST be able to create, edit, and delete tutorial sections through the admin dashboard. Tutorial content MUST support Markdown and HTML formats with dynamic variable replacement for site-specific values.
...
- Follows patterns from: `replace-homepage-faq-with-admin-managed-content`
- Uses existing route from: `add-inline-tutorial-page`
- Similar to: `SettingsFAQ.jsx`, `SettingsAnnouncements.jsx` implementations

## Technical Notes

- Uses existing `option` table, no database schema changes
- Backend validation in `controller/option.go` with new case
- New validation file: `setting/console_setting/tutorial.go`
- Frontend component: `web/src/pages/Setting/Dashboard/SettingsTutorial.jsx`
- Modified component: `web/src/pages/Tutorial/index.jsx` for content loading
