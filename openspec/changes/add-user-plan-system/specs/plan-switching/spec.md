...
The system SHALL block API requests when the user's only available plan is locked.

#### Scenario: Only plan is locked
- **GIVEN** user has only one plan and it is locked
- **WHEN** user makes an API request
- **THEN** system returns 403 with "Your plan is locked: {locked_reason}"
- **AND** no channel selection occurs

#### Scenario: Current plan locked with alternatives
- **GIVEN** user has plan A (current, locked) and plan B (not locked, has quota)
- **WHEN** user makes an API request
- **THEN** system auto-selects plan B (next available)
- **AND** plan B becomes current
