# Spec Delta: User Registration Plan Binding
...
### Requirement: Registration Flow Extension
The user registration flow SHALL include trial plan binding step after user creation.

#### Scenario: Registration flow order
- **GIVEN** trial plan "trial" exists
- **WHEN** new user registers
- **THEN** step 1: user record SHALL be created
- **AND** step 2: default token SHALL be generated (if enabled)
- **AND** step 3: trial plan SHALL be bound
- **AND** registration SHALL complete successfully
