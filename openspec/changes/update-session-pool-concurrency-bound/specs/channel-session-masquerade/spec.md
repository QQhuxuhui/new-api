...
This capability defines how Claude channel requests mask the `metadata.user_id` field using pre-generated session pools bound to channel concurrency limits, preventing upstream detection of multi-user relay patterns.
...
|-----------|------|---------|-------------|
| Default Session Count | Integer | 5 | Sessions when no concurrency limit set |
| Rotation Period | Duration | 2 hours | Time between session rotations |
| Sessions Per Rotation | Integer | 1 | How many sessions to replace each period |

## Related Capabilities

- `channel-management`: Channel configuration including `MaxConcurrentRequestsPerKey`
- `concurrency-limit-retry-behavior`: How concurrency limits affect request routing
