...
The `/tutorial` page must display admin-configured content when available, with proper rendering based on the configured format.
...
The frontend must replace dynamic variables in tutorial content with actual site-specific values.
...
The tutorial page must provide appropriate fallback behavior when admin hasn't configured content.
...
Tutorial content in Markdown format must be rendered as formatted HTML with proper styling.
...
HTML content must be sanitized to prevent XSS and other security vulnerabilities.
...
Tutorial content should be cached on the frontend to improve performance.
...
Tutorial content must be displayed responsively across different screen sizes.
