...
func SetApiRouter(router *gin.Engine) {
	...
	apiRouter.Use(middleware.GlobalAPIRateLimit())
	{
		...
		apiRouter.GET("/poster", controller.GetPoster)
		...
		userRoute := apiRouter.Group("/user")
		{
			...
			adminRoute.Use(middleware.AdminAuth())
			{
				...
				adminRoute.GET("/manage/aff-review", controller.GetAffReviewList)
				adminRoute.POST("/manage/aff-audit-logs/:log_id/approve", controller.ApproveAuditLogHandler)
				adminRoute.POST("/manage/aff-audit-logs/:log_id/reject", controller.RejectAuditLogHandler)
		...
		optionRoute.Use(middleware.RootAuth())
		{
			...
			optionRoute.POST("/billing_expr/preview", controller.PreviewBillingExpr)
			optionRoute.POST("/migrate_console_setting", controller.MigrateConsoleSetting) // 用于迁移检测的旧键，下个版本会删除
			optionRoute.POST("/poster/upload", middleware.UploadRateLimit(), controller.UploadPoster)
		}
		...
		analyticsRoute.Use(middleware.AdminAuth())
		{
			...
			analyticsRoute.GET("/user-topups/:user_id", controller.GetUserTopUpHistory)
			analyticsRoute.GET("/user-plan-orders/:user_id", controller.GetUserPlanOrders)
