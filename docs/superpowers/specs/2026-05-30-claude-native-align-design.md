...
2. **与模拟缓存耦合**:强依赖。仅在 `session_prefix` 模拟缓存已开启且本请求生效(`CacheSimulationApplied`)时激活,缓存数字复用现有引擎,扣费走现有逻辑。
...
- 模拟缓存已开启且本请求生效(`info.CacheSimulationApplied == true`,mode 为 `session_prefix`)

任一不满足 → 原样透传上游字节。`native_align` 开了但模拟缓存未开/非 session_prefix → 复用现有 `warnUnsupportedCacheSimulationMode` 模式,一次性 WARN,不生效。

billing 不额外改动:扣费抵扣仍由模拟缓存的 `CacheSimulationApplied` 现有逻辑负责(`service/quota.go:341`)。
...
因此:**native_align 激活时,在 message_start 处即运行一次 cache sim**(total input tokens 在 message_start 的 `Message.Usage.InputTokens` 已知),将缓存切分结果缓存到 `claudeInfo`,message_start 与 message_delta 复用同一份。
...
- 缓存数字来自 message_start 处运行的 cache sim
